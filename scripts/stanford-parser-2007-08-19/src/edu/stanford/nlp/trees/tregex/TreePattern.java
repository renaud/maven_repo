 ...
 * a <code>tgrep</code>-type pattern.  Instances of
 * it can be matched against instances of the {@link Tree} class.
...
public class TreePattern {
  ...
  public static TreePattern compile(String str) {
    ...
    List tokens = Arrays.asList(str.split("\\s+"));
  ...
  private static List separateNames(List tokens) {
    List newTokens = new ArrayList(tokens.size());
  ...
  private static List separateParens(List tokens) {
    ...
    for (Iterator i = tokens.iterator(); i.hasNext();) {
      ...
      List rightParensList = new ArrayList();
  ...
  static NegationDescriptionPair formatDescriptionString(String str) {
    ...
    result += alternates[alternates.length - 1];
