 ...
 * {@link MapLabel <code>MapLabel</code>} labels at each node, which
...
public class TreeGraphNode extends Tree {
  ...
  private static LabelFactory mlf = MapLabel.factory();
  ...
  public TreeGraphNode(Label label) {
    this.label = (MapLabel) mlf.newLabel(label);
  }
  ...
  public TreeGraphNode(Tree t, TreeGraphNode parent) {
    ...
    this.label = (MapLabel) mlf.newLabel(t.label());
  }
   ...
   */
  public void setLabel(final MapLabel label) {
    this.label = label;
  }
   ...
   *
   * @param arcLabel the <code>Object</code> with which the new arc
   *                 is to be labeled.
  ...
  public boolean addArc(Object arcLabel, TreeGraphNode node) {
    ...
    Map map = label.map();
    if (!map.containsKey(arcLabel)) {
      map.put(arcLabel, new HashSet());
    }
    return ((Set) map.get(arcLabel)).add(node);
  }
  ...
  public Set followArcToSet(Object arcLabel) {
    Map map = ((MapLabel) label()).map();
    if (!map.containsKey(arcLabel)) {
      return null;
    }
    // now make sure it's a Set...
    Object value = map.get(arcLabel);
  ...
  public Set arcLabelsToNode(TreeGraphNode destNode) {
    Set arcLabels = new HashSet();
    for (Iterator it = ((MapLabel) label()).map().entrySet().iterator(); it.hasNext();) {
      Map.Entry arc = (Map.Entry) it.next();
      Object val = arc.getValue();
      if (val != null && val instanceof Set && ((Set) val).contains(destNode)) {
        Object key = arc.getKey();
        if (key != null) {
          arcLabels.add(key);
        }
      }
  ...
  public Object arcLabelToNode(TreeGraphNode destNode) {
    Set arcLabels = arcLabelsToNode(destNode);
    if (arcLabels == null) {
      return null;
    }
   ...
   * in this node's {@link MapLabel <code>MapLabel</code>} and the
   * <code>MapLabel</code>s of all its descendants.<p>
   * <p/>
   ...
   * MapLabel <code>MapLabel</code>}.  (In contrast to {@link
   * edu.stanford.nlp.ling.CategoryWordTag
  ...
  public TreeGraphNode headWordNode() {
    TreeGraphNode hwn = safeCast(label.headWord());
   ...
   * storing it in this node's {@link MapLabel
   * <code>MapLabel</code>}.  (In contrast to {@link
   * edu.stanford.nlp.ling.CategoryWordTag
   * <code>CategoryWordTag</code>}, we store head words and head
   * tags as references to nodes, not merely as
   * <code>String</code>s.)
  ...
  public TreeGraphNode headTagNode() {
    TreeGraphNode htn = safeCast(label.headTag());
   ...
   * edu.stanford.nlp.ling.MapLabelFactory
   * <code>MapLabelFactory</code>} will be used.  The factories
