  ...
  */
  ...
  { // initialization block
    ...
    for (int i = 0; i < arabicChars.length; i++) {
      Character ca = new Character(arabicChars[i]);
      Character cb = new Character(buckChars[i]);
      a2b.put(ca, cb);
      b2a.put(cb, ca);
    }
    if (DEBUG) {
      unmappable = new Counter<String>();
    }
  }
  ...
  private String convert(String in, boolean unicodeToBuckwalter) {
    ...
    for (int i = 0, sz = in.length(); i < sz; i++) {
      Character inCh = new Character(in.charAt(i));
