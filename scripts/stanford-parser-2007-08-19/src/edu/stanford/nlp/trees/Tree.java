...
public abstract class Tree extends AbstractCollection<Tree> implements Label, Labeled, Scored, Serializable {
  ...
  public List<Tree> getChildrenAsList() {
    ...
    ArrayList<Tree> kidsList = new ArrayList<Tree>(length);
  ...
  public void setChildren(List<Tree> childTreesList) {
    if (childTreesList == null || childTreesList.size() == 0) {
  ...
  public IntPair getSpan() {
    return (IntPair) ((MapLabel) label()).get(MapLabel.SPAN_KEY);
  }
  ...
  private int constituentsNodes(int left) {
    if (isLeaf()) {
      ...
      MapLabel l = (MapLabel) label();
      l.put(MapLabel.SPAN_KEY, new IntPair(left, left));
      return (left + 1);
    } else {
      int position = left;
      ...
      MapLabel l = (MapLabel) label();
      l.put(MapLabel.SPAN_KEY, new IntPair(left, position - 1));
  ...
  public Set<Dependency> dependencies(HeadFinder hf) {
    return dependencies(Filters.acceptFilter(), hf);
  }
  ...
  /**
   * Return a set of MapLabel-MapLabel dependencies, represented as
   * Dependency objects, for the Tree.
   *
   * @param f  Dependencies are excluded for which the Dependency is not
   *           accepted by the Filter
   * @param hf The HeadFinder to use to identify the head of constituents.
   *           If <code>hf</code> is not <code>null</code>, the code assumes
   *           that it can use <code>headPreTerminal(hf)</code> to find a
   *           tag and word to make a maplabel.  This won't work in some
   *           cases, such as for dependency trees.
   *           If <code>hf</code> is <code>null</code>, then nodes are
   *           required to already store their head word and tag, accessible
   *           via the {@link HasWord} and {@link HasTag} interfaces.
   * @return Set of dependencies (each a <code>Dependency</code> between two
   *           <code>MapLabel</code>s, which each contain a tag(), word(),
  ...
  public Set<Dependency> mapDependencies(Filter<Dependency> f, HeadFinder hf) {
    ...
    for (Tree node : this) {
      ...
      if (hf != null) {
        ...
        if (hwt != null) {
          ml = new MapLabel(hwt.firstChild().label());
          ml.setWord(hwt.firstChild().label().value());
          ml.setTag(hwt.label().value());
        } else {
          throw new IllegalArgumentException("mapDependencies: headFinder failed!");
        }
      } else {
        if (l instanceof HasWord && l instanceof HasTag) {
          ml = new MapLabel(((HasWord) l).word());
          ml.setWord(((HasWord) l).word());
          ml.setTag(((HasTag) l).tag());
        } else {
          throw new IllegalArgumentException("mapDependencies: no head info in label!");
        }
      }
      // System.err.println("Made maplabel: " + ml.toString("value{map}"));
      ...
      for (Tree child : node.children()) {
        ...
        if (hf != null) {
          Tree dwt = child.headPreTerminal(hf);
          if (dwt != null) {
            dml = new MapLabel(dwt.firstChild().label());
            dml.setWord(dwt.firstChild().label().value());
            dml.setTag(dwt.label().value());
          }
        } else {
          if (dl instanceof HasWord && dl instanceof HasTag) {
            dml = new MapLabel(((HasWord) dl).word());
            dml.setWord(((HasWord) dl).word());
            dml.setTag(((HasTag) dl).tag());
          }
        }
        // System.err.println("kid is " + dl);
        // System.err.println("transformed to " + dml.toString("value{map}"));
        if (ml.value() != null && ml.tag() != null && dml != null && ml.value().equals(dml.value()) && ml.tag().equals(dml.tag()) && ! seenHead) {
          seenHead = true;
        } else {
          Dependency p = new UnnamedDependency(ml, dml);
          if (f.accept(p)) {
            deps.add(p);
          }
        }
   ...
   * @return a filtered copy of the tree.
   */
  public Tree prune(final Filter filter) {
   ...
   * This will only work if the leaves extend AbstractMapLabel.
   */
  ...
  private int indexLeaves(int startIndex) {
    if (isLeaf()) {
      AbstractMapLabel afl = (AbstractMapLabel) label();
      int oldIndex = afl.index();
      if (oldIndex >= 0) {
        startIndex = oldIndex;
      } else {
      ...
      Tree[] kids = children();
      for (int i = 0; i < kids.length; i++) {
        startIndex = kids[i].indexLeaves(startIndex);
      }
    }
    return startIndex;
  }
