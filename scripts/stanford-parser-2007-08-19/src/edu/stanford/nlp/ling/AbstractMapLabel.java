 ...
 * @author grenager
 */
public abstract class AbstractMapLabel implements Label, HasWord, HasTag, HasCategory, HasContext, Serializable {
  ...
  protected AbstractMapLabel(MapFactory mapFactory) {
    if (mapFactory==null) {
      this.mapFactory = MapFactory.HASH_MAP_FACTORY;
    } else {
      this.mapFactory = mapFactory;
    }
    this.map = this.mapFactory.newMap(initCapacity);
  }
