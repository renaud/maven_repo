...
public class ChineseCharacterBasedLexicon implements Lexicon {
  ...
  public static void printStats(Collection<Tree> trees) {
    Counter<Integer> wordLengthCounter = new Counter<Integer>();
    Counter<TaggedWord> wordCounter = new Counter<TaggedWord>();
    Counter<Symbol> charCounter = new Counter<Symbol>();
    int counter = 0;
    for (Tree tree : trees) {
      counter++;
      ...
      for (int i = 0, size = taggedWords.size(); i < size; i++) {
        ...
        wordLengthCounter.incrementCount(new Integer(word.length()));
    ...
    Set singletonChars = charCounter.keysBelow(1.5);
    Set singletonWords = wordCounter.keysBelow(1.5);

    Counter<String> singletonWordPOSes = new Counter<String>();
    for (Iterator singletonWordIter = singletonWords.iterator(); singletonWordIter.hasNext();) {
      TaggedWord taggedWord = (TaggedWord) singletonWordIter.next();
    ...
    Counter singletonCharRads = new Counter();
    for (Iterator singletonCharIter = singletonChars.iterator(); singletonCharIter.hasNext();) {
      Symbol s = (Symbol) singletonCharIter.next();
      singletonCharRads.incrementCount(new Character(RadicalMap.getRadical(s.getCh())));
    }
  ...
  public void train(Collection<Tree> trees) {
    ...
    Counter charCounter = new Counter();
    ...
    Set singletons = charCounter.keysBelow(1.5);
    knownChars = new HashSet(charCounter.keySet());
    ...
    Counter POSCounter = new Counter();
    List context = new ArrayList(CONTEXT_LENGTH + 1);
    for (Iterator iterator = trees.iterator(); iterator.hasNext();) {
      Tree tree = (Tree) iterator.next();
      //      tree.pennPrint();
      List words = tree.taggedYield();
      for (Iterator wordIter = words.iterator(); wordIter.hasNext();) {
        TaggedWord taggedWord = (TaggedWord) wordIter.next();
        String word = taggedWord.word();
    ...
    for (int i = 0; i <= CONTEXT_LENGTH; i++) {
      ...
      for (Iterator it = counterEntries.iterator(); it.hasNext();) {
        Map.Entry<List,Counter> entry = (Map.Entry<List,Counter>) it.next();
        context = entry.getKey();
        Counter c = entry.getValue();
        Distribution thisPrior = charDistributions.get(context.subList(0, context.size() - 1));
        double priorWeight = thisPrior.getNumberOfKeys() / 200;
  ...
  public float score(IntTaggedWord iTW, int loc) {
    ...
    List charList = new ArrayList(chars.length + CONTEXT_LENGTH + 1);
  ...
  public String sampleFrom(String tag) {
    ...
    Distribution d = getBackedOffDist(context);
    Symbol gen = (Symbol) d.sampleFrom();
    genLoop:
  ...
  public String sampleFrom() {
    String POS = (String) POSDistribution.sampleFrom();
  ...
  private Distribution getWordLengthDistribution() {
    ...
    while (samples++ < 10000) {
      String s = sampleFrom();
      c.incrementCount(new Integer(s.length()));
      if (samples % 1000 == 0) {
        System.out.print(".");
      }
    }
    System.out.println();
    Distribution genWordLengthDist = Distribution.getDistribution(c);
    return genWordLengthDist;
  }

  public static void main(String[] args) throws IOException {
    Map<String,Integer> flagsToNumArgs = new HashMap<String, Integer>();
    flagsToNumArgs.put("-parser", new Integer(3));
    flagsToNumArgs.put("-lex", new Integer(3));
    flagsToNumArgs.put("-test", new Integer(2));
    flagsToNumArgs.put("-out", new Integer(1));
    flagsToNumArgs.put("-lengthPenalty", new Integer(1));
    flagsToNumArgs.put("-penaltyType", new Integer(1));
    flagsToNumArgs.put("-maxLength", new Integer(1));
    flagsToNumArgs.put("-stats", new Integer(2));
  ...
  private static class Symbol implements Serializable {
    ...
    public static Interner interner = new Interner();
