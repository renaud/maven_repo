...
public class EquivalenceClassEval {
  ...
  * by default.*/
  ...
  public void eval(Collection guesses, Collection golds, PrintWriter pw) {
    ...
    Pair precision = evalPrecision(guesses, golds);
    previousGuessed = (Counter) precision.first();
    guessed.addAll(previousGuessed);
    previousGuessedCorrect = (Counter) precision.second();
    guessedCorrect.addAll(previousGuessedCorrect);

    if (verbose) {
      System.out.println("evaluating recall...");
    }
    Pair recall = evalPrecision(golds, guesses);
    previousGold = (Counter) recall.first();
    gold.addAll(previousGold);
    previousGoldCorrect = (Counter) recall.second();
    goldCorrect.addAll(previousGoldCorrect);
  }

  /* returns a Pair of each */
  Pair evalPrecision(Collection guesses, Collection golds) {
    Collection internalGuesses = null;
    Collection internalGolds = null;
    if(bagEval) {
    ...
    Counter thisGuessed = new Counter();
    Counter thisCorrect = new Counter();
    for (Object o : internalGuesses) {
      Object equivalenceClass = eq.equivalenceClass(o);
  ...
  public Counter lastPrecision() {
    ...
    result.addAll(previousGuessedCorrect);
    result.divideBy(previousGuessed);
    return result;
  }
  ...
  public Counter lastRecall() {
    Counter result = new Counter();
    result.addAll(previousGoldCorrect);
    result.divideBy(previousGold);
    return result;
  }
  ...
  public Counter lastF1() {
    Counter result = new Counter();
    Set keys = Sets.union(previousGuessed.keySet(),previousGold.keySet());
    for(Object key : keys) {
  ...
  public static void main(String[] args) {
    ...
    Collection guesses = Arrays.asList(new String[]{"S:a", "S:b", "VP:c", "VP:d", "S:a"});
    Collection golds = Arrays.asList(new String[]{"S:a", "S:b", "S:b", "VP:d", "VP:a"});
  ...
  static class Eval {
    ...
    public Eval(EqualityChecker e) {
      this(false,e);
    }

    public Eval() {
      this(DEFAULT_CHECKER);
    }

    public Eval(boolean bagEval) {
      this(false,DEFAULT_CHECKER);
    }

    public Eval(boolean bagEval, EqualityChecker e) {
    ...
    static class CollectionContainsChecker {
      ...
      public CollectionContainsChecker(EqualityChecker e) {
        this.e = e;
      }
    ...
    public double evalPrecision(Collection guesses, Collection golds) {
      ...
      for (Iterator i = internalGuesses.iterator(); i.hasNext();) {
        Object o = i.next();
        thisGuessed += 1.0;
        if (checker.contained(o, internalGolds)) {
  ...
  public Factory factory() {
    return new Factory() {
      ...
      EquivalenceClasser eq1 = eq;
      Eval.CollectionContainsChecker checker1 = checker;
      String summaryName1 = summaryName;

      public EquivalenceClassEval equivalenceClassEval() {
        EquivalenceClassEval e = new EquivalenceClassEval(eq1, checker1, summaryName1);
